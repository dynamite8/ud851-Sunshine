...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        ...
        if (mCursor.move(position)) {
            ...
            int high = mCursor.getInt(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));
            int low = mCursor.getInt(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));
            ...
            if (SunshineDateUtils.isDateNormalized(date)) {
                dateString = SunshineDateUtils.getFriendlyDateString(mContext, date, false);
            }

            String highLowString = SunshineWeatherUtils.formatHighLows(mContext, high, low);
            String conditon = SunshineWeatherUtils.getStringForWeatherCondition(mContext, weatherId);
            String weatherForThisDay = dateString + " - " + conditon + " - " + highLowString;
